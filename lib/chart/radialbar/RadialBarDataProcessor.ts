...
export function processRadialBarData(csvData: string, config: RadialBarChartConfig): any[] {
  ...
  return data.map((row, index) => ({
    ...
    data: [
      {
        x: cleanString(row[idColumn]) || `item-${index}`,
        y: toNumber(row[valueColumn])
      }
    ]
  })).filter(item => item.data[0].y > 0); // Only include items with positive values
}
...
// Validate CSV for radial bar chart
export function validateCsvForRadialBar(csvData: string, config: RadialBarChartConfig): { valid: boolean; missingColumns: string[]; availableColumns: string[] } {
